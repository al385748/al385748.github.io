...
function endSceneTransition(value){
	...
	if(value > 10){
		...
		document.getElementById("touchAnywhere").style.opacity = value/2 + "%";
		setTimeout(function () {
...
function returnHome(){
	...
	if(currentScene!="intro"){
		...
		if(!circleTransitionWorking && !isSelectable){
			...
			if(!visitedScreensArray.includes(currentScene)){
				visitedScreensArray.push(currentScene);
			...
			setTimeout(function () {
				...
				console.log("RETURNEANDO A " + newScene + " DESDE " + currentScene);
...
function downloadPostmortem(){
	window.open("docs/POSTMORTEM.pdf");
}